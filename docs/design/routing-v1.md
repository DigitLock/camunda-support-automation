...
- **T-1008** (Phase 5.5): `intent = cancel_refund`, `customerTier = standard`, neutral Russian text, `bookingValue = 450 EUR` (below the threshold), `customerCurrency = TRY` (an ECB currency — RSD is not served by the FX gateway), `bookingRef = BK-90`. The mock Booking API only knows `BK-77`, `BK-81`, `BK-90`, `BK-1001` and an unknown ref raises `BOOKING_NOT_FOUND` (found live on the first 5.5 run with `BK-1008`); `BK-90` is the one seed T-1001…T-1007 never mutate (T-1004 carries it but takes the answer branch). Consequence: routing uses the ticket's 450 EUR, the refund uses the mock's value for `BK-90` (210 EUR → TRY) — both below the threshold, so the expectation is unaffected. Expected: `refunds / normal / 24`, `requiredChecks = ["refund-policy"]`, path as T-1002 incl. `convert-refund`. Exists for the Russian notify path, not for a routing rule.
