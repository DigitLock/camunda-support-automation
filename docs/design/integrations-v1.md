...
| ID | Decision | Rationale |
|---|---|---|
| D4-1 | Error contract: Booking API 2xx → complete with `{bookingStatus}`; 404 **or missing/null `bookingRef`** → BPMN error `BOOKING_NOT_FOUND`; 5xx or client timeout (10 s) → fail with `retries - 1` and errorMessage. **Amended in Phase 6.1:** transport errors join the 5xx/timeout path; the failure carries `retryBackOff` (`RETRY_BACKOFF`, default `PT10S`) and an errorMessage naming the call and the retries left, so the incident text in Operate is self-explanatory and an outage that ends within *retries × backoff* heals without an incident (`operations-v1.md` §3) | Business absence is a modelled path, infrastructure trouble is a retry and then an incident; the missing-ref case cannot reach the API and is business absence too |
