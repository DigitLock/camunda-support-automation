...
| 5.5 | Acceptance, docs, screenshots. Also: `review-classification` form — the Text view renders live form values, so after the agent changes a select the block labelled "LLM classification" shows the corrected value, not the LLM snapshot; rename the block or drop `intent`/`sentiment` from it (audit row is unaffected: `llm_intent` comes from `llm_audit`). One RU ticket in `tests/e2e/tickets.json` (cancel_refund) so notify-RU and the refund amount in Russian text are covered on the stand | planned |