...
| 6.2 | Scenario B: process v9 with `BOOKING_NOT_FOUND` error boundary events (owner), live incident resolved by instance migration v8 → v9 (`--migrate-probe`), timeout handling (`BK-FAIL-TIMEOUT`), runbook `docs/runbooks/incidents.md` | planned |
| 6.3 | SLA escalation: `slaOverride` in the start event and the `slaDeadline` mapping (D6-3), non-interrupting timer boundary `sla-timer` on `handle-by-agent` (D6-5), T-1009 in `tickets.json`, `--probe-sla` | planned — same v9 |
